...
const getPeriodsPerYear = (frequency) => PERIODS_PER_YEAR[frequency];
const getBasePayForRun = (employee, draft) => {
    const periods = getPeriodsPerYear(draft.payFrequency);
    const salaryOverrideAmount = Math.max(0, draft.salaryOverrideAmount ?? 0);
    const hourlyRate = employee.employmentType === "hourly"
...
export const inferVacationHandling = (draft) => {
...
export const estimateVacationPayoutForRun = (employee, draft) => {
...
const getBracketTax = (taxableIncome, brackets) => {
...
const calculateOntarioHealthPremium = (annualIncome, taxYear) => {
...
const calculateOntarioSurtax = (basicProvincialTax, taxYear) => {
...
const calculateOntarioTaxReduction = (ontarioTaxBeforeReduction, taxYear) => {
...
const calculateAnnualIncomeTax = (annualTaxableIncome, annualBaseCppContribution, annualEiPremium, employee, taxYear) => {
    ...
    const federalCredits = (Math.max(employee.federalClaimAmount, 0) + rateTable.federalCanadaEmploymentCredit) * rateTable.federalCreditRate
        + federalCppEiCredits;
    ...
    const provincialTaxAnnual = Math.max(0, ontarioTaxBeforeReduction - taxReduction + healthPremium);
    return { federalTaxAnnual, provincialTaxAnnual };
};
const calculateAnnualGross = (employee, draft) => {
    const periods = getPeriodsPerYear(draft.payFrequency);
...
export const calculatePayroll = (employee, draft, priorYtd = createZeroTotals(), taxYear = getTaxYearFromDraft(draft)) => {
    const taxTable = getTaxTable(taxYear);
    const rateTable = taxTable.rates;
    const periods = getPeriodsPerYear(draft.payFrequency);
    ...
    const cpp2Annual = cppExempt
        ? 0
        : Math.min(Math.max(0, Math.min(annualizedGross, rateTable.yampe) - rateTable.ympe) * rateTable.cpp2, rateTable.cpp2Max);
    ...
    const cpp2BandThisPeriod = Math.max(0, Math.min(grossBeforeCurrent + periodPensionable, rateTable.yampe) - Math.max(grossBeforeCurrent, rateTable.ympe));
    ...
    const annualPeriodicTaxableIncome = Math.max(0, (periodicIncome - rrspRppPrppContribution - unionDues - f5a) * periods
        - (employee.prescribedZoneDeductionAnnual ?? 0)
        - (employee.otherAnnualDeductionsAnnual ?? 0));
    const annualTaxableIncome = Math.max(0, annualPeriodicTaxableIncome + priorYtd.bonusAmount + priorYtd.vacationPaid + nonPeriodicIncome - f5b);
    const annualTaxWithoutCurrentNonPeriodic = calculateAnnualIncomeTax(Math.max(0, annualPeriodicTaxableIncome + priorYtd.bonusAmount + priorYtd.vacationPaid), annualBaseCppContribution, eiAnnual, employee, taxYear);
    const annualTaxWithCurrentNonPeriodic = calculateAnnualIncomeTax(annualTaxableIncome, annualBaseCppContribution, eiAnnual, employee, taxYear);
    const federalTax = roundMoney(annualTaxWithoutCurrentNonPeriodic.federalTaxAnnual / periods
        + Math.max(0, annualTaxWithCurrentNonPeriodic.federalTaxAnnual - annualTaxWithoutCurrentNonPeriodic.federalTaxAnnual));
    const provincialTax = roundMoney(annualTaxWithoutCurrentNonPeriodic.provincialTaxAnnual / periods
    ...
    const totalDeductions = roundMoney(rrspRppPrppContribution + unionDues + cpp + cpp2 + ei + federalTax + provincialTax);
