...
import type { Client, CompanyProfile, Employee, PayRunDraft, PayRunRecord } from "../../src/types.js";
import {
  addClient,
  addEmployee,
  deleteClient,
...
apiRouter.put("/company-profile", (request, response) => {
  const incoming = request.body as CompanyProfile;
  response.json(updateCompanyProfile(incoming));
});

apiRouter.post("/clients", (request, response) => {
  const incoming = request.body as Omit<Client, "id" | "active">;
  const client: Client = {
...
apiRouter.post("/employees", (request, response) => {
  const incoming = request.body as Omit<Employee, "id" | "provinceOfEmployment" | "active">;
...
apiRouter.post("/pay-runs/preview", (request, response) => {
  const { employeeId, draft, excludedPayRunId } = request.body as { employeeId: string; draft: PayRunDraft; excludedPayRunId?: string };
...
apiRouter.post("/pay-runs", (request, response) => {
  const draft = request.body as PayRunDraft;
