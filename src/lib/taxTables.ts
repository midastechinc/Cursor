import type { PayRunDraft, TaxTableSummary } from "../types.js";
...
export type PayrollTaxTable = {
  ...
  rates: {
    ...
    eiEmployerMax: number;
    federalCanadaEmploymentCredit: number;
    federalCreditRate: number;
    provincialCreditRate: number;
  };
  federalBrackets: TaxBracket[];
  ontarioBrackets: TaxBracket[];
  ontarioHealthPremium: TaxThreshold[];
  ontarioSurtax: {
  ...
  ontarioTaxReductionBase: number;
};

const TAX_TABLES: Record<number, PayrollTaxTable> = {
  2026: {
    summary: {
      taxYear: 2026,
...
export const getDefaultTaxYear = () =>
  Math.max(...Object.keys(TAX_TABLES).map((year) => Number(year)));

export const getTaxYearFromDraft = (draft: Pick<PayRunDraft, "payPeriodStart" | "payPeriodEnd">) => {
...
export const getTaxTableSummary = (year = getDefaultTaxYear()): TaxTableSummary => getTaxTable(year).summary;

export const getSupportedTaxYears = () =>
  Object.keys(TAX_TABLES)
